...
import { jsExams } from "../../constants/exam";
...
const ExamModal = ({ selectedExam, setSelectedExam }: ExamModalProps) => {
  ...
  const exam = jsExams.find((item) => item.id === selectedExam);

  const [timeLeft, setTimeLeft] = useState<number>((exam?.duration ?? 0) * 60);

  // Hook Stop Scroll When Modal is Open

  useLockBodyScroll(!!selectedExam);
  ...
  return (
    <div className="fixed inset-0 z-10  flex items-center justify-center bg-black/45 p-3 sm:p-6">
      <div className="w-full max-w-200 overflow-hidden rounded-4xl bg-[#5673F1] sm:max-w-3xl">
        <div className="px-3 pb-2 pt-6 text-white">
          {/* Modal Header  */}
        ...
        <div className="mt-2 rounded-t-[34px] bg-[#F8FAFF] px-6 pb-8 pt-4 sm:mx-3 sm:px-6">
          {question && (
            <div>
              <h2 className="mt-4 text-lg lg:text-[34px] sm:text-md font-semibold text-[#252A3E]">
                  ...
                  return (
                    <li key={idx} className="flex px-4 ">
                      <button
                        ...
                        className={`flex w-full items-center overflow-hidden rounded-xl border text-left  ${
                          isSelected
                      ...
                      >
                        ...
                        <span className="px-5 text-2xl lg:text-4xl sm:text-md font-medium leading-none">
                          {option}
                        </span>
                      </button>
                    </li>
